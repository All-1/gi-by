...
function write_kitchen_content($kitchen){	
	...
	$sql_texture = "SELECT DISTINCT type FROM gi_designer_textures WHERE kitchens LIKE '%$kitchen%' ORDER BY FIELD (type, 'Массив, шпон', 'МДФ', 'FENIX', 'Crystal', 'Brilliant', 'Skin', 'Syncron', 'Egger', 'Постформинг', 'Столешница')";
	...
	foreach ($result_texture as $row_texture){
		...
		switch ($type){
			case "Постформинг": $typetext = $lang_adm->df_tabletops; break;
			case "Массив, Шпон": $typetext = $lang_adm->df_mass_shpon; break;
			case "Мдф": $typetext = $lang_adm->df_mdf; break;
			default: $typetext = $type; break;
		}
