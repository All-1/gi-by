			...
			<div style='width:calc(100% - 40px); padding:20px; min-height:100px; border:1px solid #dcdcdc; box-shadow: 2px 2px 2px rgba(1,1,1,0.2); margin:20px 0;'>
				...
				<form method='post' enctype='multipart/form-data'>
					...
					<div style='float:left; width:calc(100% - 340px); margin-left:20px;'>
						...
						<b>Выберите валюту:</b><br>
						<select name='currency'>
							<option value='Бел.руб.'>Бел. руб.</option>
							<option value='Рос.руб.'>Рос. руб.</option>
							<option value='Гривен'>Гривна</option>
						</select>
	...
	if(isset($_POST['moderate'])){
		...
		if(isset($_FILES['images']) and $_FILES['images']==TRUE and $_FILES['images']['name'][0]!=''){
			...
			for ($i=0; $i<=$numberAddFile; $i++) {
				...
				$imagelink = $images_folder .  $salon_id . "_" . $imagesname_data . "_" . basename($_FILES['images']['name'][$i]);
				$thumblink = $thumbs_folder . $salon_id . "_" . $imagesname_data . "_" . basename($_FILES['images']['name'][$i]);				
				...
				$image_name = $salon_id . "_" . $imagesname_data . "_" . basename($_FILES['images']['name'][$i]);
				$thumb_name = $salon_id . "_" . $imagesname_data . "_" . basename($_FILES['images']['name'][$i]);
