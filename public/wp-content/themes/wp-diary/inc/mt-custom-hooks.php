...
if( ! function_exists( 'wp_diary_scroll_top_content' ) ) :
	...
	function wp_diary_scroll_top_content() {
        echo '<div id="mt-scrollup" class="animated arrow-hide">'. __( '', 'wp-diary' ) .'</div>';
	}
...
if( ! function_exists( 'wp_diary_innerpage_header_title' ) ) :
	...
	<?php
			...
			echo '<div class="entry-title">'. esc_html( '404 Error', 'wp-diary' ) .'</div>';
		} elseif( is_home() ) {
			$page_for_posts_id = get_option( 'page_for_posts' );
			$page_title = get_the_title( $page_for_posts_id );
	?>
			<div class="entry-title"><?php echo esc_html( $page_title ); ?></div>
	<?php
		}
	}
