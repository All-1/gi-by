...
var EditOverlayAction = LeafletToolbar.ToolbarAction.extend({
	...
	ToggleRotateDistort = EditOverlayAction.extend({
		initialize: function(map, overlay, options) {
			...
			options = options || {};
			options.toolbarIcon = {
				html: '<span class="fa fa-compress' + '"></span>',
				tooltip: 'Resize',
				title: 'Resize'
			};
...
L.DistortableImage.Edit = L.Handler.extend({
  ...
  addHooks: function() {
    ...
    if (this._mode === "lock") {
      ...
      this._mode = "scale";
      map.addLayer(this._scaleHandles);
      this._scaleHandles.eachLayer(function(layer) {
        layer.setOpacity(0);
  ...
  _toggleScale: function() {
    ...
    map.removeLayer(this._handles[this._mode]);
		...
		if (this._mode === "scale") { this._mode = "scale"; } 
		else { this._mode = "scale"; }
...
L.Map.BoxSelectHandle = L.Map.BoxZoom.extend({
  ...
  _onMouseMove: function (e) {
    ...
    box.style.width = (Math.max(0, Math.abs(offset.x) - 0)) + 'px';
    box.style.height = (Math.max(0, Math.abs(offset.y) - 0)) + 'px';
  },
